...
def get_terrain_color(noise_value, rainfall, plant_density):
    ...
    # elif plant_density > 400:
    #     plant_factor = max(0, (150 - plant_density) / 100)#max(0, 1 - (plant_density - 100) / 100)  # Gradually turns brown again
    # else:
    #     plant_factor = 1  # Most green in 50-100 range
    if plant_density < 50:
        plant_factor = 0  # No green before 95
    elif plant_density > 50:
        plant_factor = 1  # Fully green above 100
    else:
        plant_factor = min(1, (plant_density - 95) / (100 - 95))  # Gradually increases to max at 100

    # Adjust rainfall effect
    if rainfall < 80000:
        rain_factor = 0  # No blue for very low rainfall
    elif rainfall > 100000:
        rain_factor = (100000 - 80000) / (90000 - 80000)  # rain_factor = max(0, 1 - (rainfall - 100000) / 10000)  # Gradually fades out above 90000
    else:
...
while running:
    ...
    crop = dependent_variables.get("Crop Yield")
